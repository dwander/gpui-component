...
/// 현재 창의 툴팁을 억제한다 — [`WindowState::suppress_tooltip`] 의 앱용 입구.
///
/// `WindowState` 는 크레이트 내부 타입이라 앱에서 직접 부를 수 없다.
pub fn suppress_tooltip(window: &Window, cx: &mut App) {
    WindowState::suppress_tooltip(window, cx);
}

/// [`suppress_tooltip`] 으로 걸었던 툴팁 억제를 해제한다.
pub fn restore_tooltip(window: &Window, cx: &mut App) {
    WindowState::restore_tooltip(window, cx);
}
